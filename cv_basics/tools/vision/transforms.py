...
import PIL
import numpy as np
from .functional import permute, normalize, horizontal_flip, vertical_flip, resize_long,resize, brightness, contrast, saturation, hue
...
class Resize:
    ...
    def __call__(self, im: np.ndarray):
        ...
        im = resize(im, self.target_size, self.interpolation_dict[interpolation])
        return im
...
class Normalize_Gray:
    ...
    def __init__(self, mean: 0.5, std: 0.5, channel_first: bool = False):
        self.mean = mean
        self.std = std
        self.channel_first = channel_first
